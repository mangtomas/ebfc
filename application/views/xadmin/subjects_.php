...
<legend style="font: bold 26px 'arial';"><i class="custom-icon-cog" style="margin-top:-3px"></i><?=(strtolower($action)=='edit') ? 'Update' : $action ?> Subject</legend>
<div class="alert alert-info"><?=(strtolower($action)=='edit') ? 'Update' : $action ?> Subject to your course</div>
<p><span class="required">*</span> Indicates fields are required</p>
<form action="<?=base_url('xadmin/subjects/'.strtolower($action))?>" method="POST" class="form-horizontal" id="validate-form">
			<?php
			if(strtolower($action)=='edit'){
				?>
				<input type="hidden" name="subject_id" id="subject_id" value="<?=$result['subject_id']?>" />
				<div class="" id="email-container">
					 <div class="control-group">
					  ...
					  <div class="control-group">
						...
						<div class="controls">
						  <input type="text" id="subject" name="subject" value="<?=$result['subject']?>" class="alphanumeric span5" style="float:left"> <span class="validation-status pull-left"></span>
						</div>
					  </div>
					   <div class="control-group">
					 	<label class="control-label" for="subject_code">Subject Code<span class="required">*</span></label>
						<div class="controls">
						  <input type="text" id="subject_code" name="subject_code" value="<?=$result['subject_code']?>" class="span2 alphanumeric" style="float:left"><span class="validation-status pull-left"></span>
						</div>
					  </div>
					  <div class="control-group">
				...
				<div class="control-group">
					   ...
					   <div class="control-group">
						<label class="control-label" for="subject">Subject <span class="required">*</span></label>
						<div class="controls">
						  <input type="text" id="subject" name="subject" class="span5 alphanumeric" style="float:left"><span class="validation-status pull-left"></span>
						</div>
					  </div>
					   <div class="control-group">
					 	<label class="control-label" for="subject_code">Subject Code<span class="required">*</span></label>
						<div class="controls">
						  <input type="text" id="subject_code" name="subject_code" class="span2 alphanumeric" style="float:left"><span class="validation-status pull-left"></span>
						</div>
					  </div>
	...
	var validator = $("#validate-form").validate({
		rules: {
			course_id:{
				...
				required: true,
				remote: {
			        url: "<?=base_url()?>xadmin/subjects/validate/ajax=true/?current=<?=isset($result['subject']) ? $result['subject'] : null?>&mode=<?=strtolower($action)?>",
			        type: "post",
			        data: {
			          course_id: function() {
